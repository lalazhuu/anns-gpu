...
struct GpuSearchResult_v4 { float dist; int id; };

const int TILE_WIDTH_V4 = 16;
__global__ void matrix_mul_shared_mem_kernel(const float* base, const float* query, float* results, int n, int m, int d); // Declaration

template <int K_LOCAL>
__global__ void find_topk_kernel_approx_v4_tmpl(const float* results_matrix, GpuSearchResult_v4* topk_results,
                                                int n, int m, int k) {
...
template <int K_LOCAL>
std::vector<GpuSearchResult_v4>
gpu_flat_search_v4(const float* base, const float* query, size_t n, size_t m, size_t d, size_t k) {
    ...
    float *d_base, *d_query, *d_results; GpuSearchResult_v4 *d_topk_results;
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_topk_results, m * k * sizeof(GpuSearchResult_v4)));
    ...
    dim3 blockDimMatMul(TILE_WIDTH_V4, TILE_WIDTH_V4);
    ...
    CUDA_CHECK(cudaEventRecord(start));
    find_topk_kernel_approx_v4_tmpl<K_LOCAL><<<gridDimTopK, blockDimTopK, shared_mem_size>>>(d_results, d_topk_results, n, m, k);
    ...
    std::vector<GpuSearchResult_v4> h_topk_results(m * k);
    CUDA_CHECK(cudaEventRecord(start));
    CUDA_CHECK(cudaMemcpy(h_topk_results.data(), d_topk_results, m * k * sizeof(GpuSearchResult_v4), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaEventRecord(stop)); CUDA_CHECK(cudaEventSynchronize(stop)); CUDA_CHECK(cudaEventElapsedTime(&time_D2H, start, stop));
    
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Performance Breakdown (v4 - Approx Top-K, K_LOCAL=" << K_LOCAL << "):" << std::endl;
    std::cout << std::fixed << std::setprecision(3);
    std::cout << "1. Host to Device (H2D) Transfer: " << time_H2D << " ms" << std::endl;
    std::cout << "2. MatMul Kernel (Shared Mem):    " << time_MatMul << " ms" << "   (Recall=1.0)" << std::endl;
...
__global__ void matrix_mul_shared_mem_kernel(const float* base, const float* query, float* results, int n, int m, int d) {
    __shared__ float tile_base[TILE_WIDTH_V4][TILE_WIDTH_V4];
    __shared__ float tile_query[TILE_WIDTH_V4][TILE_WIDTH_V4];
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    int row = blockIdx.y * blockDim.y + ty;
    int col = blockIdx.x * blockDim.x + tx;
    float sum = 0.0f;
    for (int t = 0; t < (d + TILE_WIDTH_V4 - 1) / TILE_WIDTH_V4; ++t) {
        if (row < n && (t * TILE_WIDTH_V4 + tx) < d) {
            tile_base[ty][tx] = base[row * d + t * TILE_WIDTH_V4 + tx];
        } else { tile_base[ty][tx] = 0.0f; }
        if (col < m && (t * TILE_WIDTH_V4 + ty) < d) {
            tile_query[ty][tx] = query[col * d + t * TILE_WIDTH_V4 + ty];
        } else { tile_query[ty][tx] = 0.0f; }
        __syncthreads();
        for (int k_tile = 0; k_tile < TILE_WIDTH_V4; ++k_tile) {
