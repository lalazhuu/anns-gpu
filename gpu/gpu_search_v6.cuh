...
struct GpuSearchResult_v6 { float dist; int id; };

const int TILE_WIDTH_V6 = 16;

__global__ void matrix_mul_shared_mem_kernel(const float* base, const float* query, float* results, int n, int m, int d) {
    __shared__ float tile_base[TILE_WIDTH_V6][TILE_WIDTH_V6];
    __shared__ float tile_query[TILE_WIDTH_V6][TILE_WIDTH_V6];
    int tx = threadIdx.x; int ty = threadIdx.y;
    int row = blockIdx.y * blockDim.y + ty; int col = blockIdx.x * blockDim.x + tx;
    float sum = 0.0f;
    for (int t = 0; t < (d + TILE_WIDTH_V6 - 1) / TILE_WIDTH_V6; ++t) {
        if (row < n && (t * TILE_WIDTH_V6 + tx) < d) { tile_base[ty][tx] = base[row * d + t * TILE_WIDTH_V6 + tx]; } else { tile_base[ty][tx] = 0.0f; }
        if (col < m && (t * TILE_WIDTH_V6 + ty) < d) { tile_query[ty][tx] = query[col * d + t * TILE_WIDTH_V6 + ty]; } else { tile_query[ty][tx] = 0.0f; }
        __syncthreads();
        for (int k_tile = 0; k_tile < TILE_WIDTH_V6; ++k_tile) { sum += tile_base[ty][k_tile] * tile_query[k_tile][tx]; }
        __syncthreads();
    }
...
__global__ void find_topk_kernel_flexible_sample(const float* results_matrix, GpuSearchResult_v6* topk_results, int n, int m, int k) {
...
template <int K_LOCAL, int SAMPLE_NUMERATOR, int SAMPLE_DENOMINATOR>
std::vector<GpuSearchResult_v6>
gpu_flat_search_v6(const float* base, const float* query, size_t n, size_t m, size_t d, size_t k) {
    ...
    float *d_base, *d_query, *d_results; GpuSearchResult_v6 *d_topk_results;
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_topk_results, m * k * sizeof(GpuSearchResult_v6)));
    ...
    dim3 blockDimMatMul(TILE_WIDTH_V6, TILE_WIDTH_V6);
    ...
    CUDA_CHECK(cudaEventRecord(start));
    find_topk_kernel_flexible_sample<K_LOCAL, SAMPLE_NUMERATOR, SAMPLE_DENOMINATOR><<<gridDimTopK, blockDimTopK, shared_mem_size>>>(d_results, d_topk_results, n, m, k);
    ...
    std::vector<GpuSearchResult_v6> h_topk_results(m * k);
    CUDA_CHECK(cudaEventRecord(start));
    CUDA_CHECK(cudaMemcpy(h_topk_results.data(), d_topk_results, m * k * sizeof(GpuSearchResult_v6), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaEventRecord(stop)); CUDA_CHECK(cudaEventSynchronize(stop)); CUDA_CHECK(cudaEventElapsedTime(&time_D2H, start, stop));
    
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Performance Breakdown (v6 - Flexible Sample, K_LOCAL=" << K_LOCAL << ", RATE=" << SAMPLE_NUMERATOR << "/" << SAMPLE_DENOMINATOR << "):" << std::endl;
    ...
    std::cout << "3. Top-K Kernel (Flexible Approx):" << time_TopK << " ms" << std::endl;
